...
import com.dmyk.android.telephony.DmykImplTelephonyManager;
...
public class DmykTelephonyManager extends IDmykTelephony.Stub {
    ...
    private static final int MSG_BLUETOOTH_DISCONNECT = 1;
    private static final int MSG_BLUETOOTH_CONNECT = 2;
    private static final int DEFAULT_BATTERY_TEMPERATURE = 260;
    ...
    private DmykTelephonyManager(PhoneGlobalsEx app, Context context) {
        mApp = app;
        mContext = context;
        mBluetoothConnector = new BluetoothConnector(context);
        if (mBluetoothConnector != null) {
            mBluetoothConnector.setupBluetoothConnectionProxy();
        }

        mPhoneCount = getPhoneCount();

        mTemperature = Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.BATTERY_TEMPERATURE, DEFAULT_BATTERY_TEMPERATURE);
        Log.d(TAG, "init mTemperature =" + mTemperature);
        ...
        intentFilter.addAction(DmykAbsTelephonyManager.ACTION_5G_STATE_SETTING);
        ...
        intentFilter.addAction(BluetoothMap.ACTION_CONNECTION_STATE_CHANGED);
        intentFilter.addAction(BluetoothMapClient.ACTION_CONNECTION_STATE_CHANGED);
        intentFilter.addAction(BluetoothSap.ACTION_CONNECTION_STATE_CHANGED);
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (DmykAbsTelephonyManager.ACTION_VOLTE_STATE_SETTING.equals(action)) {
                ...
                Log.d(TAG, "battery changed plugged = " + pluggedIn);
                Intent chargeStateInt = new Intent(DmykAbsTelephonyManager.ACTION_BATTERY_CHARGE_STATE_CHANGE);
                chargeStateInt.setPackage(DmykAbsTelephonyManager.PACKAGE_NAME);
                chargeStateInt.putExtra(DmykAbsTelephonyManager.EXTRA_BATTERY_CHARGE_STATE, pluggedIn ? 1 : 0);
                mContext.sendBroadcast(chargeStateInt);

                //get Battery Temperature;
                int temperature = intent.getIntExtra(EXTRA_TEMPERATURE, 0);

                Log.d(TAG, "original temperature =" + mTemperature + " current temperature =" + temperature);
                if (mTemperature != temperature) {
                   setTemperature(temperature);
                }
            } else if (DmykAbsTelephonyManager.ACTION_5G_STATE_SETTING.equals(action)) {
                int phoneId = intent.getIntExtra(DmykAbsTelephonyManager.EXTRA_SIM_PHONEID, -1);
                Log.d(TAG, "ACTION_5G_STATE_SETTING phoneId: "  + phoneId);
                Intent nrStateSetting = new Intent(Intent.ACTION_MAIN);
                ComponentName mobileNetworkSettingsComponent = new ComponentName(
                        context.getString(R.string.mobile_network_settings_package),
                        context.getString(R.string.mobile_network_settings_class));
                nrStateSetting.setComponent(mobileNetworkSettingsComponent);
                nrStateSetting.putExtra(Settings.EXTRA_SUB_ID, getSubId(phoneId));
                nrStateSetting.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                mContext.startActivity(nrStateSetting);
            } else if (BluetoothAdapter.ACTION_STATE_CHANGED.equals(action)) {
                final int state = intent.getIntExtra(BluetoothAdapter.EXTRA_STATE,BluetoothAdapter.ERROR);
                Log.d(TAG, "BluetoothAdapter.ACTION_STATE_CHANGED state = " + state);
                //when state is on,getBluetooth info return null, need waiting for ACTION_CONNECTION_STATE_CHANGED

                if (BluetoothAdapter.STATE_ON == state && !mBluetoothConnector.isSetuped()) {
                    mBluetoothConnector.setupBluetoothConnectionProxy();
                }
                if ( BluetoothAdapter.STATE_OFF == state) {
                    Log.d(TAG,"bluetooth turn off and inform sdk");
                    sendBTConnChangeBroadcastToSdk();
                }
                       ...
                       BluetoothA2dpSink.ACTION_CONNECTION_STATE_CHANGED.equals(action) ||
                       BluetoothHidDevice.ACTION_CONNECTION_STATE_CHANGED.equals(action)||
                       BluetoothHidHost.ACTION_CONNECTION_STATE_CHANGED.equals(action) ||
                       BluetoothAvrcpController.ACTION_CONNECTION_STATE_CHANGED.equals(action) ||
                       BluetoothPan.ACTION_CONNECTION_STATE_CHANGED.equals(action) ||
                       BluetoothMap.ACTION_CONNECTION_STATE_CHANGED.equals(action) ||
                       BluetoothMapClient.ACTION_CONNECTION_STATE_CHANGED.equals(action) ||
                       BluetoothSap.ACTION_CONNECTION_STATE_CHANGED.equals(action) ||
                       BluetoothPbapClient.ACTION_CONNECTION_STATE_CHANGED.equals(action)) {
                final int state = intent.getIntExtra(BluetoothProfile.EXTRA_STATE,BluetoothProfile.STATE_DISCONNECTED);
                Log.d(TAG, "bluetooth device is connected or disconnected and inform sdk, state = " + state);
                switch (state) {
                    case BluetoothProfile.STATE_DISCONNECTED:
                        /** when disconnecting bluetooth, it will receive ACTION_CONNECTION_STATE_CHANGED four times,
                         * getBluetoothInfo still can get connected devices during 3th ACTION_CONNECTION_STATE_CHANGED,
                         * we should delay it to inform cmcc sdk; Another case is that bluetooth will disconnect 4s automatically
                         * after phonedevice connected. try to sleep 3000ms to walkround sdk 5 seconds broadcast interval limit
                         */
                         mBluetoothHandler.removeMessages(MSG_BLUETOOTH_DISCONNECT);
                         Message disconnMsg = mBluetoothHandler.obtainMessage(MSG_BLUETOOTH_DISCONNECT);
                         mBluetoothHandler.sendMessageDelayed(disconnMsg,INTERVAL_TIME);
                         Log.d(TAG,"STATE_DISCONNECTED");
                         break;
                     case BluetoothProfile.STATE_CONNECTED:
                         mBluetoothHandler.removeMessages(MSG_BLUETOOTH_CONNECT);
                         Message connMsg = mBluetoothHandler.obtainMessage(MSG_BLUETOOTH_CONNECT);
                         mBluetoothHandler.sendMessageDelayed(connMsg,INTERVAL_TIME);
    ...
    public void setTemperature(int temperature) {
        Log.d(TAG,"setTemperature temp = " + temperature);
        Settings.Global.putInt(mContext.getContentResolver(), Settings.Global.BATTERY_TEMPERATURE, temperature);
    ...
    private void sendBTConnChangeBroadcastToSdk() {
        Log.d(TAG, "sendBTConnChangeBroadcastToSdk");
        Intent btStateSetInt = new Intent(DmykAbsTelephonyManager.ACTION_BLUETOOTH_CONNECTIVITY_CHANGE);
        btStateSetInt.setPackage(DmykAbsTelephonyManager.PACKAGE_NAME);
        mContext.sendBroadcast(btStateSetInt);
    }

    private class BluetoothConnector {
    ...
    public void setupBluetoothConnectionProxy() {
        ...
        for (Integer profile : mProfilesToConnect) {
            mBluetoothAdapter.getProfileProxy(mCtx, mProfileListener, profile);
        }
        mSetuped = true;
    }
    ...
    public void closeBluetoothConnectionProxy() {
        ...
        if (mBluetoothHidHost != null) {
            mBluetoothAdapter.closeProfileProxy(BluetoothProfile.HID_HOST, mBluetoothHidHost);
        }
        if (mBluetoothPbap != null) {
            mBluetoothAdapter.closeProfileProxy(BluetoothProfile.PBAP, mBluetoothPbap);
        }
        if (mBluetoothMap != null) {
    ...
    public boolean isBluetoothDeviceConnected(BluetoothDevice btDevice) {
        if (mBluetoothHeadset != null && mBluetoothHeadset.getConnectionState(btDevice) == BluetoothProfile.STATE_CONNECTED) {
            Log.d(TAG, "headset is connected");
            return true;
        }

        if (mBluetoothA2dp != null && mBluetoothA2dp.getConnectionState(btDevice) == BluetoothProfile.STATE_CONNECTED) {
            Log.d(TAG, "a2dp is connected");
            return true;
        }

        if (mBluetoothHidHost != null && mBluetoothHidHost.getConnectionState(btDevice) == BluetoothProfile.STATE_CONNECTED) {
            Log.d(TAG, "hid-host is connected");
            return true;
        }

        if (mBluetoothPan != null && mBluetoothPan.getConnectionState(btDevice) == BluetoothProfile.STATE_CONNECTED) {
            Log.d(TAG, "pan is connected");
            return true;
        }

        if (mBluetoothPbap != null && mBluetoothPbap.getConnectionState(btDevice) == BluetoothProfile.STATE_CONNECTED) {
            Log.d(TAG, "pbap is connected");
            return true;
        }

        if (mBluetoothMap != null && mBluetoothMap.getConnectionState(btDevice) == BluetoothProfile.STATE_CONNECTED) {
            Log.d(TAG, "map is connected");
            return true;
        }
        Log.d(TAG, "isBluetoothDeviceConnected return false");
        return false;
    }
    ...
    public boolean isBluetoothConnected(BluetoothDevice bluetoothDevice) {
        return mBluetoothConnector != null
                && mBluetoothConnector.isBluetoothDeviceConnected(bluetoothDevice);
    }
}
