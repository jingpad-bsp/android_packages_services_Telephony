...
public class CallForwardHelper {
    ...
    private static final int EVENT_QUERY_FORWARDING = 103;
    private static final int EVENT_QUERY_DELAY_MILLIS = 10 * 1000;
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (DISSMISS_VIDEO_CF_NOTIFICATION_ACTION.equals(action) ||
                ...
                if (intent.getIntExtra(CarrierConfigManagerEx.CARRIER_CONFIG_CHANGED_TYPE, -1) == CONFIG_CHANGED_SUB) {
                    ...
                    if (cfi != null && imsManager != null) {
                        cfi.mShowVideoStatus = (imsManager.isVolteEnabledByPlatform() || imsManager.isWfcEnabledByPlatform())
    ...
    private class CallForwardPhoneStateListener extends PhoneStateListener {
        ...
        public void onServiceStateChanged(ServiceState serviceState) {
            ...
            if (serviceState.getState() == ServiceState.STATE_IN_SERVICE
                ...
                int activeSimCount = mSubscriptionManager.getActiveSubscriptionInfoCount();
                CallForwardHelperInfo cfi = mCallForwardHelperInfoMap.get(this.mSubId);

                if (cfi != null && cfi.mNeedQuery && ((activeSimCount == 1)
                        || otherPhoneInService(this.mSubId))) {
                    queryAllCfu(cfi);
                    if (mHandler.hasMessages(EVENT_QUERY_FORWARDING)) {
                        mHandler.removeMessages(EVENT_QUERY_FORWARDING);
                    }
                    mHandler.sendMessageDelayed(mHandler.obtainMessage(EVENT_QUERY_FORWARDING),
                            EVENT_QUERY_DELAY_MILLIS);
                }
            }
        }
    };

    private boolean otherPhoneInService(int currentSubId) {
        boolean otherPhoneInService = false;
        for (Phone phone : PhoneFactory.getPhones()) {
            if (mSubscriptionManager.getPhoneId(currentSubId) == phone.getPhoneId()) continue;
            if (phone.getServiceStateTracker() != null
                    && phone.getServiceStateTracker().getServiceState() != null) {
                otherPhoneInService = (phone.getServiceStateTracker().getServiceState()
                        .getDataRegState() == ServiceState.STATE_IN_SERVICE)
                        || (phone.getServiceStateTracker().getServiceState()
                                .getState() == ServiceState.STATE_IN_SERVICE);
            }
        }

        return otherPhoneInService;
    }
    ...
    private void queryAllCfu(CallForwardHelperInfo cfi) {
        ...
        if(mApplication.getApplicationContext()
            ...
            if (mTelephonyManager.isDataEnabled()
                ...
                synchronized (mPendingCfuQuery) {
                    if (!mPendingCfuQuery.contains(cfi)) {
    ...
    private void doPendingQuery() {
        ...
        synchronized (mPendingCfuQuery) {
            ...
            if (it.hasNext()) {
                ...
                Log.d(TAG, "doPendingQuery:, cfi.mNeedQuery = " + cfi.mNeedQuery);
                if(cfi.mNeedQuery) {
                    doQuery(cfi);
                }
            }
    ...
    private void sortPendingQuery() {
        int dds = mSubscriptionManager.getDefaultDataSubscriptionId();
        Log.d(TAG, "sortPendingQuery:, dds = " + dds);
        if (!SubscriptionManager.isValidSubscriptionId(dds)) {
            return;
        }
        synchronized (mPendingCfuQuery) {
            for (int i = 0; i < mPendingCfuQuery.size(); i++) {
                if (mPendingCfuQuery.get(i).mSubId == dds) {
                    if (i == 0) {
                        break;
                    } else {
                        // switch mPendingCfuQuery[i] and mPendingCfuQuery[0]
                        Collections.swap(mPendingCfuQuery, 0, i);
                        break;
                    }
                }
            }
